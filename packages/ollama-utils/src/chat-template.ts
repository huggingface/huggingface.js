const CUSTOM_TEMPLATE_MAPPING: ((ggufTmpl: string) => { ollamaTmpl: string; stop?: string } | undefined)[] = [
	(ggufTmpl: string) =>
...
function mapGGUFTemplateToOllama(
	gguf: NonNullable<ModelInfo["gguf"]>,
	debugModelId?: string
		...
		if (matched) {
			...
			if (debugModelId) {
				void Pulse.channel(
					"ollama-registry",
					"tmpl-custom-map",
					`🔍 Custom map Jinja to Go:\n\n\`\`\`${matched.ollamaTmpl}\`\`\`\n\nhttps://hf.co/api/models/${debugModelId}`,
					{ markdown: true }
				);
			}
			return {
...
export function convertJinjaToGoTemplate(gguf: NonNullable<ModelInfo["gguf"]>):
	| {
			tmpl:  string;
			stop?: string;
	  }
	...
	try {
		...
		const format = (msgs: { role: string; content: string }[], retried = false) => {
			try {
				return jinja.render({
					messages:              msgs,
