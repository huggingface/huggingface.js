...
export function isStructuredDataInput<TOutput>(
	...
	return isObject(sample) && "structured_data" in sample && isObject(sample.structured_data) && Object.values(sample.structured_data).every(val => typeof val === "number" || typeof val === "string");
}
...
function _isZeroShotTextInput<TOutput>(
	...
	return isObject(sample) && "candidate_labels" in sample && typeof sample.candidate_labels === "string" && "multi_class" in sample && typeof sample.multi_class === "boolean";
}
...
export function isSentenceSimilarityInput<TOutput>(
	...
	return isObject(sample) && "source_sentence" in sample && typeof sample.candidate_labels === "string" && "sentences" in sample && isStrArray(sample.sentences);
}
