...
import { initOneShot, claimInput, appendInput, bufferedInput, runOneShot, ONESHOT_MAX_INPUT } from "./wasm-oneshot.js";
import type { OneShotOwner } from "./wasm-oneshot.js";
 ...
 */
export class Hasher implements OneShotOwner {
	...
	/**
	 * Fast one-shot mode: message bytes are buffered directly in wasm memory
	 * and the whole hash runs in a single wasm call at finalize time.
	 * Active from reset()/construction until the message outgrows the wasm
	 * staging area (or an incompatible operation forces the scalar path).
	 */
	private fastActive: boolean;
	private fastLen: number;
	...
	constructor(keyWords?: Uint32Array, flags?: number) {
		...
		this.fastActive = IS_LITTLE_ENDIAN && initOneShot();
		this.fastLen = 0;
	}
	...
	/**
	 * Called when another hasher (or one-shot hash call) claims the wasm input
	 * staging area: replay our buffered bytes through the scalar path.
	 */
	evictOneShot(): void {
		if (!this.fastActive) {return;}
		this.fastActive = false;
		if (this.fastLen > 0) {
			const buffered = bufferedInput(this.fastLen);
			this.fastLen = 0;
			this.updateScalar(buffered);
		}
	}
	...
	update(input: Uint8Array): this {
		if (this.fastActive) {
			if (this.fastLen + input.length <= ONESHOT_MAX_INPUT) {
				claimInput(this);
				appendInput(input, this.fastLen);
				this.fastLen += input.length;
				return this;
			}
			// Message outgrew the one-shot staging area: replay buffered bytes
			// through the scalar streaming path and continue there.
			this.evictOneShot();
		}
		return this.updateScalar(input);
	}

	private updateScalar(input: Uint8Array): this {
		let inputOffset = 0;
	...
	private finalizeOutput(): {
		...
		// Fast mode buffers bytes in wasm memory; replay them through the scalar
		// state so the generic output-block machinery (XOF, >32B output) works.
		if (this.fastActive) {
			this.evictOneShot();
		}
		// The merge loop below walks the CV stack; restore its length afterwards
		// so that finalize() is non-destructive and can be called repeatedly.
		const savedCvStackLen = this.cvStackLen;
		try {
			return this.finalizeOutputInner();
		} finally {
			this.cvStackLen = savedCvStackLen;
		}
	}

	private finalizeOutputInner(): {
		inputCv: Uint32Array;
		blockWords: Uint32Array;
		blockLen: number;
		counter: number;
	...
	finalize(outputLength: number = OUT_LEN): Uint8Array {
		if (this.fastActive && outputLength <= 32) {
			// One-shot fast path: whole message is buffered in wasm memory;
			// a single wasm call computes the entire tree + root.
			claimInput(this);
			const result = new Uint8Array(outputLength);
			runOneShot(this.keyWords, this.flags, this.fastLen, result, outputLength);
			return result;
		}
		...
		if (outputLength <= 64) {
			...
			// Note: finalizeOutput() drains the CV stack, so the XofReader must be
			// built from the output we already computed (calling finalizeXof() here
			// would re-run finalizeOutput() on the drained state and corrupt the
			// result for multi-chunk messages).
			const xof = new XofReader(
			...
			result.set(xof.read(outputLength));
		}
