...
import { initOneShot, claimInput, appendInput, runOneShot, ONESHOT_MAX_INPUT } from "./wasm-oneshot.js";
...
function transposeBlocksToSimd(
	...
	const allFull =
		blockCount === 4 && blockLens[0] === 64 && blockLens[1] === 64 && blockLens[2] === 64 && blockLens[3] === 64;
		...
		if (len === 64) {
			...
			if (inputWords && off % 4 === 0) {
				...
				for (let w = 0; w < 16; w++) {
					...
					mem32[w * 4 + b] =
						input[srcOff] | (input[srcOff + 1] << 8) | (input[srcOff + 2] << 16) | (input[srcOff + 3] << 24);
				}
			}
		} else if (len === 0) {
			// Zero block
	...
	if (inputWords && chunkOffsets[0] % 4 === 0) {
		...
		for (let pos = 0; pos < 16; pos++) {
			...
			for (let w = 0; w < 16; w++) {
				...
				mem32[dstBase] = input[off0] | (input[off0 + 1] << 8) | (input[off0 + 2] << 16) | (input[off0 + 3] << 24);

				// Chunk 1
				const off1 = chunkOffsets[1] + blockByteOff + wordByteOff;
				mem32[dstBase + 1] = input[off1] | (input[off1 + 1] << 8) | (input[off1 + 2] << 16) | (input[off1 + 3] << 24);

				// Chunk 2
				const off2 = chunkOffsets[2] + blockByteOff + wordByteOff;
				mem32[dstBase + 2] = input[off2] | (input[off2 + 1] << 8) | (input[off2 + 2] << 16) | (input[off2 + 3] << 24);

				// Chunk 3
				const off3 = chunkOffsets[3] + blockByteOff + wordByteOff;
				mem32[dstBase + 3] = input[off3] | (input[off3 + 1] << 8) | (input[off3 + 2] << 16) | (input[off3 + 3] << 24);
			}
		}
	}
}
	...
	while (chunkIdx < numChunks) {
		...
		if (canUseBatchPath) {
			...
			for (let blockIdx = 0; blockIdx < 16; blockIdx++) {
				...
				if (blockLens[0] === 0 && blockLens[1] === 0 && blockLens[2] === 0 && blockLens[3] === 0) {continue;}
...
export function hash(input: Uint8Array, outputLength: number = OUT_LEN): Uint8Array {
	// Fast path: single-call wasm one-shot engine (transpose + tree + root in wasm)
	if (outputLength <= 32 && input.length <= ONESHOT_MAX_INPUT && IS_LITTLE_ENDIAN && initOneShot()) {
		claimInput(null);
		appendInput(input, 0);
		const result = new Uint8Array(outputLength);
		runOneShot(IV, 0, input.length, result, outputLength);
		return result;
	}
	// Outputs > 64 bytes need the XOF machinery of the Hasher
	if (outputLength > 64) {
		return new Hasher().update(input).finalize(outputLength);
	}
 ...
 */
export function hashInto(input: Uint8Array, output: Uint8Array, outputLength: number = OUT_LEN): void {
	// Validate output buffer
	if (output.length < outputLength) {
		throw new Error(`Output buffer too small: ${output.length} < ${outputLength}`);
	}

	// Fast path: single-call wasm one-shot engine
	if (outputLength <= 32 && input.length <= ONESHOT_MAX_INPUT && IS_LITTLE_ENDIAN && initOneShot()) {
		claimInput(null);
		appendInput(input, 0);
		runOneShot(IV, 0, input.length, output, outputLength);
		return;
	}

	// Outputs > 64 bytes need the XOF machinery of the Hasher
	if (outputLength > 64) {
		output.set(new Hasher().update(input).finalize(outputLength));
		return;
	}
