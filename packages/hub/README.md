...
On Node >= 26, whose built-in `fetch` negotiates HTTP/2 with small flow-control windows, xet downloads force HTTP/1.1 by default (like the Hub server does), which keeps large downloads at full speed. Passing your own `fetch` disables this.
