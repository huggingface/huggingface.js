...
import { createChunker, finalize, nextBlock, getChunks, hashToHex, hexToBytes, xorbHash, fileHash, hmac, verificationHash } from "../src/index.js";
...
describe("xetchunk-wasm", () => {
	...
	describe("Hash functions (verified against Rust thin-wasm)", () => {
		...
		function makeData(size: number, a: number, b: number): Uint8Array {
			...
			for (let i = 0; i < size; i++) data[i] = (i * a + b) & 0xff;
			return data;
		}
		...
		it("verificationHash", () => {
			...
			expect(hashToHex(verificationHash(hashes))).toBe("82d580dd3896919ef870af78b2c7bd21e67e79777922bbc8cef1fc21630c308c");
		});
