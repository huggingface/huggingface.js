...
describe("xetchunk-wasm", () => {
	...
	describe("Minimum chunk size regression", () => {
		it("matches xet-core after a gear-hash trigger before the minimum", () => {
			...
			// Values below were generated by xet-core from the same bytes.
			const referenceChunks = getChunks(data);
			expect(referenceChunks).toHaveLength(52);
			expect(referenceChunks.slice(0, -1).every((chunk) => chunk.length >= 8192)).toBe(true);
			expect(referenceChunks[44].length).toBe(131072);
			expect(hashToHex(referenceChunks[44].hash)).toBe(
				"8ee1833ff616a01e187888865c8e22f403c860b7a6c347ba28c4511995875086",
			);

			for (const blockSize of [4096, 1024 * 1024]) {
				const chunker = createChunker();
				const chunks: Chunk[] = [];
				for (let pos = 0; pos < data.length; pos += blockSize) {
					chunks.push(...nextBlock(chunker, data.subarray(pos, Math.min(pos + blockSize, data.length))));
				}
				const finalChunk = finalize(chunker);
				if (finalChunk) {
				...
				expect(chunks.map((chunk) => chunk.length)).toEqual(referenceChunks.map((chunk) => chunk.length));
				expect(chunks.map((chunk) => hashToHex(chunk.hash))).toEqual(
					referenceChunks.map((chunk) => hashToHex(chunk.hash)),
				);
			}
		});
	});
	...
	describe("Triggering data test", () => {
		it("should handle triggering data patterns correctly", () => {
			...
			ref_cb[1] = [8320, 16576, 24832, 33088, 41344, 49600, 57856, 65536];
			...
			ref_cb[12] = [8324, 16584, 24844, 33104, 41364, 49624, 57884, 65536];
			...
			ref_cb[17] = [8329, 16594, 24859, 33124, 41389, 49654, 57919, 65536];
			...
			ref_cb[24] = [8272, 16480, 24688, 32896, 41104, 49312, 57520, 65536];
			...
			ref_cb[29] = [8228, 16549, 24870, 33191, 41512, 49833, 58154, 65536];
			...
			ref_cb[53] = [8209, 16535, 24861, 33187, 41513, 49839, 58165, 65536];
			...
			ref_cb[61] = [8380, 16696, 25012, 33328, 41644, 49960, 58276, 65536];
			...
			ref_cb[75] = [8387, 16710, 25033, 33356, 41679, 50002, 58325, 65536];
			...
			ref_cb[78] = [8304, 16544, 24784, 33024, 41264, 49504, 57744, 65536];
			...
			ref_cb[84] = [8332, 16600, 24868, 33136, 41404, 49672, 57940, 65536];
			data_sample_at_11111[85] = 69;
			ref_cb[85] = [8371, 16678, 24985, 33292, 41599, 49906, 58213, 65536];
			data_sample_at_11111[86] = 0;
			ref_cb[86] = [8196, 16542, 24888, 33234, 41580, 49926, 58272, 65536];
			...
			ref_cb[89] = [8310, 16556, 24802, 33048, 41294, 49540, 57786, 65536];
			...
			ref_cb[104] = [8416, 16768, 25120, 33472, 41824, 50176, 58528, 65536];
	...
	describe("Hash functions (verified against Rust thin-wasm)", () => {
		// Reference data generated by running Rust thin-wasm on deterministic inputs

		function makeData(size: number, a: number, b: number): Uint8Array {
		...
		it("xorbHash: 200KB (2 chunks)", () => {
			const chunks = getChunks(makeData(200000, 7, 13));
			expect(hashToHex(xorbHash(chunks))).toBe("f0755d96ee199d4f01e83da9fe2c38353de7fd38fb79a320a7d50be510026f74");
		});

		it("xorbHash: 1MB (8 chunks)", () => {
			const chunks = getChunks(makeData(1000000, 13, 37));
			expect(hashToHex(xorbHash(chunks))).toBe("7be289d851485c40dcde52dbcf37a7823a5ef0bfcdf1ca1c5a32ad9594c71571");
		});

		it("fileHash: 200KB", () => {
			const chunks = getChunks(makeData(200000, 7, 13));
			expect(hashToHex(fileHash(chunks))).toBe("d88d9e0b760f32c90b7c6592b4cb85251c01bb797371e3c637087b96ac578723");
		});
		...
		it("fileHash: empty (no chunks) is the all-zero hash", () => {
			...
			// CAS shard validation rejects for empty files as a reconstruction hash mismatch.
			expect(hashToHex(fileHash([]))).toBe("0".repeat(64));
		});

		it("hmac", () => {
			const hash = hexToBytes("118bbac5c38b2bca9c2b7ae8307786de066696b91b2b5ccb7dbbd4608c966882");
			const key = hexToBytes("abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789");
			expect(hashToHex(hmac(hash, key))).toBe("1e1f5ae2bae99fe030647a14a8c5fe6cdce57826387a7552236dfbebed533e04");
		});

		it("verificationHash", () => {
			const hashes = [
				hexToBytes("118bbac5c38b2bca9c2b7ae8307786de066696b91b2b5ccb7dbbd4608c966882"),
				hexToBytes("7884d4de4bcc904767362a91e97eb6c2b6bc5f3a61b640d0cad8bb1129ff0827"),
			];
			expect(hashToHex(verificationHash(hashes))).toBe(
				"82d580dd3896919ef870af78b2c7bd21e67e79777922bbc8cef1fc21630c308c",
			);
		});
