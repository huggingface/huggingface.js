...
import type { Data, File, Files, Message, Tool, Update } from "./types";
import { isBlob } from "./utils/files";
import { extractJSON } from "./utils/textFormatting";
import { templatePreprompt } from "./formats/prompts/templatePreprompt";
...
export class AgentScratchpad {
	...
	constructor({ agent, tools, inputFile }: { agent: HfChatAgent; tools: Tool[]; inputFile?: Data }) {
		...
		this.updates = [];
		this.tools = tools;
		this.files = inputFile ? { input: { file: inputFile, description: "A file input by the user" } } : {};
		this.preprompt = "";
		...
		const pastFiles = this.agent.chatHistory
			...
			.reduce((acc, message) => {
				if (message.scratchpad) {
					...
					acc = [...acc, ...(keysInBody.map((key) => message.scratchpad?.files[key]) as File[])];
				}
				return acc;
			}, [] as File[]);
		...
		this.preprompt = templatePreprompt({
			tools: this.tools,
			files: this.files,
		});
	}
	...
	get formattedScratchpad(): string {
		return this.agent.chatFormat({ messages: [...this.agent.chatHistory, ...this.scratch], preprompt: this.preprompt });
	}
			...
			while (tools < this.maxTools) {
				...
				if (toolInput["tool"] === "finalAnswer") {
					...
					if (matchingFiles.length === 0) {
						...
						input = this.files[matchingFiles[0]].file;
					}
					...
					if (isBlob(toolOutput)) {
						this.files["tool-" + tools] = { file: toolOutput, description: "An input generated by " + tool.name };
						files = this.files;
