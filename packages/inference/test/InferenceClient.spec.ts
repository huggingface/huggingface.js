...
describe.skip("InferenceClient", () => {
	...
	describe.concurrent(
		"Boundless",
		() => {
			const client = new InferenceClient(env.HF_BOUNDLESS_KEY ?? "dummy");

			HARDCODED_MODEL_INFERENCE_MAPPING["boundless"] = {
				"Qwen/Qwen3.6-35B-A3B-FP8": {
					provider: "boundless",
					hfModelId: "Qwen/Qwen3.6-35B-A3B-FP8",
					providerId: "qwen3.6",
					status: "live",
					task: "conversational",
				},
			...
			it("chatCompletion", async () => {
				const res = await client.chatCompletion({
					...
					provider: "boundless",
					messages: [{ role: "user", content: "Complete this sentence with words, one plus one is equal " }],
					// The lineup reasons by default, and reasoning tokens come out of the same budget as
					// the reply: too small a cap returns an empty `content` with finish_reason "length".
					max_tokens: 512,
				});
				if (res.choices && res.choices.length > 0) {
					const completion = res.choices[0].message?.content;
